...
#Importing data
data = pd.read_csv('cbbBracket.csv', header=0,index_col='TEAM')
...
decision_tree_model = DecisionTreeClassifier('gini',random_state=1)
LDA_model = LinearDiscriminantAnalysis(solver='lsqr')
nb_model = GaussianNB()
svm_model = SVC(probability=True)

model_list = [nb_model,LDA_model,log_model,svm_model,random_forest_model,decision_tree_model,extra_trees_model]
p_dict = {}
...
#training_x = training[['BARTHAG','SEED','WAB','ADJOE','ADJDE','3P_D','TOR','WAS_SEED_AVG']]
training_y = training['POSTSEASON']

best_model.fit(training_x,training_y)

pred_train_y = best_model.predict(training_x)
...
#test_x = test[['BARTHAG','SEED','WAB','ADJOE','ADJDE','3P_D','TOR','WAS_SEED_AVG']]
...
final_data = test[['SEED','POSTSEASON']]
...
print(pred_finish)
print(pred_finish[['SEED','POSTSEASON','prediction']])
